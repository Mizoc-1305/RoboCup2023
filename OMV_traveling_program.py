...
thresholds_resqKit = [(10, 60, -10, 40, -90, -20)]
...
while(True):
    ...
    led_B.on()
    clock.tick()
    detected_flag = 0b0000
    img = sensor.snapshot()
    img.rotation_corr(0, 0, 180)#上下反転

    if area_flag == 0:
        for blob in img.find_blobs(thresholds_resqKit, pixels_threshold=4000, area_threshold=4000): #ここの値でピクセル数、面積の閾値を指定する
